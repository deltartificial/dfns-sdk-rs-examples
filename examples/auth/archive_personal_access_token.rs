use dfns_sdk_rs::{
    ...
    api::auth::types::ArchivePersonalAccessTokenRequest,
};
...
async fn main() {
    ...
    let request = ArchivePersonalAccessTokenRequest {
        token_id: "example-token-id".to_string(),
    };

    match client.auth().archive_personal_access_token(request).await {
        Ok(response) => {
