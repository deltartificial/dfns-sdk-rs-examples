...
async fn main() {
    ...
    let request = RegisterWithRecoveryRequest {
        body: RegisterWithRecoveryRequestBody {
    ...
    match client.auth().register_with_recovery(request).await {
        Ok(response) => {
            println!("Registration with recovery successful:");
            println!("\nUser Info:");
