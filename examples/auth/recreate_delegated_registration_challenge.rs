use dfns_sdk_rs::{
    ...
    api::auth::types::{RecreateDelegatedRegistrationChallengeRequest, RecreateDelegatedRegistrationChallengeRequestBody, UserInfoKind},
};
...
async fn main() {
    ...
    let request = RecreateDelegatedRegistrationChallengeRequest {
    ...
    match client.auth().recreate_delegated_registration_challenge(request).await {
        Ok(response) => {
            println!("Delegated registration challenge recreated successfully:");
        ...
        Err(e) => eprintln!("Error recreating delegated registration challenge: {:?}", e),
    }
} 