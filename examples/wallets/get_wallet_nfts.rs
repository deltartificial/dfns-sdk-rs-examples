use dfns_sdk_rs::{
    ...
    api::wallets::types::GetWalletNftsRequest,
};
...
async fn main() {
    ...
    let request = GetWalletNftsRequest {
        wallet_id: "example-wallet-id".to_string(),
    };

    match client.wallets().get_wallet_nfts(request).await {
        Ok(nfts) => println!("Wallet NFTs: {:?}", nfts),
