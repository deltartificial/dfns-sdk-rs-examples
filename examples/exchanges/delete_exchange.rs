use dfns_sdk_rs::{
    ...
    api::exchanges::types::DeleteExchangeRequest,
};
...
async fn main() {
    ...
    match client.exchanges().delete_exchange(request).await {
        Ok(_) => println!("Exchange deleted successfully"),
        Err(e) => println!("Error deleting exchange: {:?}", e),
    }
} 