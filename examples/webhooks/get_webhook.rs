use dfns_sdk_rs::{
    ...
    api::webhooks::types::GetWebhookRequest,
};
...
async fn main() {
    ...
    let request = GetWebhookRequest {
        webhook_id: "example-webhook-id".to_string(),
    };

    match client.webhooks().get_webhook(request).await {
        Ok(webhook) => println!("Webhook details: {:?}", webhook),
